...
def onMeterData(localclient, userdata, msg):
    global meterDataRaw
    meterDataRaw = bytes.fromhex(msg.payload.decode())
    #print("data: " + msg.payload.decode())
    #smlreader.add(bytes.fromhex(msg.payload.decode()))
    #sml_frame = smlreader.get_frame()
...
localclient.on_publish = on_publish
localclient.on_subscribe = on_subscribe

localclient.on_message = onMeterData
localclient.username_pw_set(config['broker']['user'], config['broker']['password'])
...
streamTopic = mqttBaseTopic + "/TFD01/" + config['tibber']['eui'] + "/sml"
